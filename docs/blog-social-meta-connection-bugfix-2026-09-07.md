...
`src/lib/socialAutomation/metaClient.ts` resolved managed Pages using only the inline Graph field `instagram_business_account`. A valid Page/Instagram link created through Facebook Page Settings may instead be represented through `connected_instagram_account`. That valid shape was being discarded, causing the callback to report that no linked Instagram account existed.
...
- Result: the new `connected_instagram_account` regression reproduced the same `No Instagram-linked Facebook Page...` error seen in production while the existing suite remained green.
