...
export const WHATSAPP_AUTOMATION_ACTION_TYPES = [
  "SEND_TEXT", "SEND_TEMPLATE", "SEND_SAVED_REPLY", "ASSIGN_CONVERSATION", "ADD_TAG",
...
export type WhatsAppAutomationJobStatus = "PENDING" | "PROCESSING" | "SUCCEEDED" | "FAILED" | "CANCELLED";
...
export const WHATSAPP_AUTOMATION_ACTION_OPTIONS: Array<{ value: WhatsAppAutomationActionType; label: string; description: string }> = [
  ...
  { value: "BRANCH", label: "Branch", description: "Create Yes / No paths using one condition." },
...
function normalizeAction(value: unknown, depth = 0): WhatsAppAutomationAction | null {
  ...
  if (action.type === "BRANCH") {
    ...
    action.thenActions = Array.isArray(row.thenActions)
      ? row.thenActions.map((item) => normalizeAction(item, depth + 1)).filter((item): item is WhatsAppAutomationAction => Boolean(item))
      : [];
    action.elseActions = Array.isArray(row.elseActions)
      ? row.elseActions.map((item) => normalizeAction(item, depth + 1)).filter((item): item is WhatsAppAutomationAction => Boolean(item))
      : [];
  }
  return action;
}
...
function validateAction(action: WhatsAppAutomationAction, label: string): string | null {
  ...
  if (action.type === "CALL_WEBHOOK") {
    try {
      if (new URL(action.value || "").protocol !== "https:") return `${label} must use an HTTPS webhook URL.`;
    } catch {
      return `${label} needs a valid HTTPS webhook URL.`;
    }
  }
  ...
  if (action.type === "BRANCH") {
    ...
    if (!(action.thenActions?.length || action.elseActions?.length)) return `${label} needs at least one Yes or No action.`;
  }
  return null;
}

export function countWhatsAppAutomationSteps(actions: WhatsAppAutomationAction[]): number {
  return actions.reduce((total, action) => total + 1 + (action.type === "BRANCH"
    ? countWhatsAppAutomationSteps(action.thenActions || []) + countWhatsAppAutomationSteps(action.elseActions || [])
    : 0), 0);
}
...
function validateActions(actions: WhatsAppAutomationAction[], path = "Action"): string | null {
  for (let index = 0; index < actions.length; index += 1) {
    ...
    if (action.type === "BRANCH") {
      const yes = validateActions(action.thenActions || [], `${label} Yes action`);
      if (yes) return yes;
      const no = validateActions(action.elseActions || [], `${label} No action`);
      if (no) return no;
    }
  }
  return null;
...
export function validateWhatsAppAutomationInput(value: Record<string, unknown>): { ok: true; value: WhatsAppAutomationInput } | { ok: false; error: string } {
  ...
  if (triggerType === "TAG_ADDED") {
    ...
    if (flattened.some((action) => action.type === "ADD_TAG" && action.value?.trim().toLowerCase() === triggerTag)) {
      return { ok: false, error: "This workflow can re-add its own trigger tag. Choose a different tag to avoid a loop." };
    }
  }
  if (triggerType === "CRM_STAGE_CHANGED") {
    const triggerStage = cleanString(triggerConfig.stage, 80).toUpperCase();
    if (flattened.some((action) => action.type === "UPDATE_CRM_STAGE" && action.value?.trim().toUpperCase() === triggerStage)) {
      return { ok: false, error: "This workflow can set the same CRM stage that triggered it. Choose a different stage to avoid a loop." };
    }
  }
  ...
  return {
    ok: true,
    value: {
      name, description, status: status as WhatsAppAutomationStatus,
      triggerType: triggerType as WhatsAppAutomationTriggerType, triggerConfig,
      conditionJoin, conditions, actions,
    },
  };
}
...
export function normalizeWhatsAppAutomationRow(row: Record<string, unknown>): WhatsAppAutomation {
  ...
  const value: WhatsAppAutomationInput = checked.ok ? checked.value : {
    name: cleanString(row.name, 80) || "Invalid automation",
    description: cleanString(row.description, 240), status: "PAUSED", triggerType: "NEW_MESSAGE", triggerConfig: {}, conditionJoin: "AND", conditions: [], actions: [{ type: "STOP" }],
  };
  return {
