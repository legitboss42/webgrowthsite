...
export const WHATSAPP_AUTOMATION_TRIGGER_TYPES = [
  ...
  "BUSINESS_HOURS", "WEBHOOK", "WHATSAPP_FLOW_STARTED", "WHATSAPP_FLOW_COMPLETED",
] as const;
...
export const WHATSAPP_AUTOMATION_ACTION_TYPES = [
  "SEND_TEXT", "ASK_QUESTION", "SEND_TEMPLATE", "SEND_SAVED_REPLY", "SEND_WHATSAPP_FLOW", "ASSIGN_CONVERSATION", "ADD_TAG",
...
export type WhatsAppAutomationQuestionOption = { id: string; title: string; description?: string };
export type WhatsAppAutomationCondition = { field: string; operator: WhatsAppAutomationConditionOperator; value: string };
export type WhatsAppAutomationAction = {
  type: WhatsAppAutomationActionType; value?: string; value2?: string; amount?: number; unit?: WhatsAppAutomationDelayUnit;
  condition?: WhatsAppAutomationCondition; thenActions?: WhatsAppAutomationAction[]; elseActions?: WhatsAppAutomationAction[];
  questionMode?: WhatsAppAutomationQuestionMode; choices?: WhatsAppAutomationQuestionOption[]; listButtonText?: string;
};
export type WhatsAppAutomationInput = {
  name: string; description: string; status: WhatsAppAutomationStatus; triggerType: WhatsAppAutomationTriggerType;
  triggerConfig: Record<string, string | number | boolean>; conditionJoin: WhatsAppAutomationConditionJoin;
  conditions: WhatsAppAutomationCondition[]; actions: WhatsAppAutomationAction[];
};
export type WhatsAppAutomation = WhatsAppAutomationInput & {
  id: string; version: number; createdByMemberId?: string; updatedByMemberId?: string; activatedAt?: string; pausedAt?: string; createdAt?: string; updatedAt?: string;
};
...
export type WhatsAppAutomationRun = {
  id: string; automationId: string; automationVersion: number; status: WhatsAppAutomationRunStatus; triggerType: string; triggerEventKey?: string;
  contactId?: string; conversationId?: string; nextActionIndex: number; startedAt?: string; completedAt?: string; errorCode?: string; errorMessage?: string; createdAt?: string; updatedAt?: string;
};
...
export type WhatsAppAutomationJob = {
  id: string; runId: string; automationId: string; status: WhatsAppAutomationJobStatus; dueAt: string; actionIndex: number; attempts: number; maxAttempts: number; lastError?: string; createdAt?: string;
};
...
export const WHATSAPP_AUTOMATION_TRIGGER_OPTIONS: Array<{ value: WhatsAppAutomationTriggerType; label: string; description: string }> = [
  ...
  { value: "WHATSAPP_FLOW_STARTED", label: "WhatsApp Flow started", description: "Runs after a tracked published WhatsApp Flow is sent to a contact." },
  { value: "WHATSAPP_FLOW_COMPLETED", label: "WhatsApp Flow completed", description: "Runs when a contact completes a tracked WhatsApp Flow. Answers are available under trigger.payload.flow.fields.*." },
];
...
export const WHATSAPP_AUTOMATION_CONDITION_FIELDS = [
  ...
  { value: "trigger.payload.flow.id", label: "Flow · local ID" }, { value: "trigger.payload.flow.metaId", label: "Flow · Meta ID" },
  { value: "trigger.payload.flow.name", label: "Flow · name" }, { value: "trigger.payload.flow.fields.service", label: "Flow answer · service" },
  { value: "trigger.payload.flow.fields.budget", label: "Flow answer · budget" }, { value: "trigger.payload.flow.fields.date", label: "Flow answer · date" },
] as const;
...
export const WHATSAPP_AUTOMATION_ACTION_OPTIONS: Array<{ value: WhatsAppAutomationActionType; label: string; description: string }> = [
  ...
  { value: "SEND_WHATSAPP_FLOW", label: "Send WhatsApp Flow", description: "Send a published Meta Flow and create a tracked Flow submission for the contact." },
...
function validateTriggerConfig(type: WhatsAppAutomationTriggerType, config: Record<string, string | number | boolean>): string | null {
  ...
  if (type === "NO_CUSTOMER_REPLY" || type === "NO_AGENT_REPLY") {
    const amount = Number(config.amount); const unit = cleanString(config.unit, 20).toUpperCase();
  ...
  if (type === "WEBHOOK") {
    const key = cleanString(config.key, 80); if (!/^[a-z0-9_-]{8,80}$/i.test(key)) return "Webhook keys must use 8–80 letters, numbers, underscores, or hyphens.";
  }
  if ((type === "WHATSAPP_FLOW_STARTED" || type === "WHATSAPP_FLOW_COMPLETED") && config.flowId && !cleanString(config.flowId, 100)) return "Choose a valid Flow.";
  return null;
}
function normalizeCondition(value: unknown): WhatsAppAutomationCondition | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const row = value as Record<string, unknown>; const field = cleanString(row.field, 120); const operator = cleanString(row.operator, 30).toUpperCase();
...
function normalizeQuestionChoice(value: unknown): WhatsAppAutomationQuestionOption | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const row = value as Record<string, unknown>; const id = cleanString(row.id, 200); const title = cleanString(row.title, 24); const description = cleanString(row.description, 72);
...
function normalizeAction(value: unknown, depth = 0): WhatsAppAutomationAction | null {
  ...
  if (row.amount !== undefined) { const amount = Number(row.amount); if (Number.isFinite(amount)) action.amount = amount; }
  const unit = cleanString(row.unit, 20).toUpperCase(); if (DELAY_UNITS.has(unit)) action.unit = unit as WhatsAppAutomationDelayUnit;
  ...
  if (action.type === "ASK_QUESTION") {
    const mode = cleanString(row.questionMode, 20).toUpperCase(); action.questionMode = mode === "LIST" ? "LIST" : "BUTTONS";
    ...
    const listButtonText = cleanString(row.listButtonText, 20); if (listButtonText) action.listButtonText = listButtonText;
  }
  return action;
}
function validateQuestion(action: WhatsAppAutomationAction, label: string) {
  ...
  const mode = action.questionMode || "BUTTONS"; const choices = action.choices || []; const max = mode === "BUTTONS" ? 3 : 10;
  ...
  for (const choice of choices) {
    ...
    if (ids.has(choice.id.toLowerCase())) return `${label} choice IDs must be unique.`; ids.add(choice.id.toLowerCase());
  ...
  if (action.value2 && !/^(custom\.[A-Za-z0-9_.-]{1,80}|email|phone|company|display_name|opt_in_status)$/.test(action.value2)) return `${label} answer field must be custom.<field> or a supported built-in contact field.`;
  return null;
}
function validateAction(action: WhatsAppAutomationAction, label: string): string | null {
  if (["SEND_TEXT", "SEND_TEMPLATE", "SEND_SAVED_REPLY", "SEND_WHATSAPP_FLOW", "ASSIGN_CONVERSATION", "ADD_TAG", "REMOVE_TAG", "UPDATE_CRM_STAGE", "ADD_INTERNAL_NOTE", "CALL_WEBHOOK"].includes(action.type) && !action.value?.trim()) return `${label} needs a value.`;
  ...
  if (action.type === "CALL_WEBHOOK") { try { if (new URL(action.value || "").protocol !== "https:") return `${label} must use an HTTPS webhook URL.`; } catch { return `${label} needs a valid HTTPS webhook URL.`; } }
  if (action.type === "DELAY") { if (!action.amount || action.amount < 1 || action.amount > 365) return `${label} needs a delay amount between 1 and 365.`; if (!action.unit) return `${label} needs a delay unit.`; }
  if (action.type === "BRANCH") { if (!action.condition) return `${label} needs a branch condition.`; if (!new Set(["EXISTS", "NOT_EXISTS"]).has(action.condition.operator) && !action.condition.value.trim()) return `${label} needs a comparison value.`; }
  return null;
}
export function countWhatsAppAutomationSteps(actions: WhatsAppAutomationAction[]): number { return actions.reduce((total, action) => total + 1 + (action.type === "BRANCH" ? countWhatsAppAutomationSteps(action.thenActions || []) + countWhatsAppAutomationSteps(action.elseActions || []) : 0), 0); }
function validateActions(actions: WhatsAppAutomationAction[], path = "Action"): string | null {
  for (let index = 0; index < actions.length; index += 1) {
    const action = actions[index]; const label = `${path} ${index + 1}`; const error = validateAction(action, label); if (error) return error;
    if (action.type === "STOP" && index !== actions.length - 1) return `${label}: Stop workflow must be the final action on its path.`;
    if (action.type === "BRANCH") { const yes = validateActions(action.thenActions || [], `${label} Yes action`); if (yes) return yes; const no = validateActions(action.elseActions || [], `${label} No action`); if (no) return no; }
  }
  return null;
}
function allActions(actions: WhatsAppAutomationAction[]): WhatsAppAutomationAction[] {
  const output: WhatsAppAutomationAction[] = []; for (const action of actions) { output.push(action); if (action.type === "BRANCH") output.push(...allActions(action.thenActions || []), ...allActions(action.elseActions || [])); } return output;
}
export function validateWhatsAppAutomationInput(value: Record<string, unknown>): { ok: true; value: WhatsAppAutomationInput } | { ok: false; error: string } {
  const name = cleanString(value.name, 80); if (name.length < 2) return { ok: false, error: "Automation name must be at least 2 characters." };
  const description = cleanString(value.description, 240); const status = cleanString(value.status, 20).toUpperCase(); if (!VALID_STATUSES.has(status)) return { ok: false, error: "Choose Draft, Active, or Paused." };
  const triggerType = cleanString(value.triggerType, 40).toUpperCase(); if (!VALID_TRIGGERS.has(triggerType)) return { ok: false, error: "Choose a valid automation trigger." };
  const triggerConfig = cleanConfig(value.triggerConfig); const triggerError = validateTriggerConfig(triggerType as WhatsAppAutomationTriggerType, triggerConfig); if (triggerError) return { ok: false, error: triggerError };
  const conditionJoin = cleanString(value.conditionJoin, 10).toUpperCase() === "OR" ? "OR" : "AND"; const rawConditions = Array.isArray(value.conditions) ? value.conditions : [];
  ...
  for (const raw of rawConditions) { const condition = normalizeCondition(raw); if (!condition) return { ok: false, error: "Each condition needs a valid field and operator." }; if (!new Set(["EXISTS", "NOT_EXISTS"]).has(condition.operator) && !condition.value) return { ok: false, error: "Each condition needs a comparison value." }; conditions.push(condition); }
  const rawActions = Array.isArray(value.actions) ? value.actions : []; if (!rawActions.length) return { ok: false, error: "Add at least one action." };
  const actions: WhatsAppAutomationAction[] = []; for (const raw of rawActions) { const action = normalizeAction(raw); if (!action) return { ok: false, error: "One or more workflow actions are invalid." }; actions.push(action); }
  ...
  if (triggerType === "TAG_ADDED") { const triggerTag = cleanString(triggerConfig.tag, 100).toLowerCase(); if (flattened.some((action) => action.type === "ADD_TAG" && action.value?.trim().toLowerCase() === triggerTag)) return { ok: false, error: "This workflow can re-add its own trigger tag. Choose a different tag to avoid a loop." }; }
  if (triggerType === "CRM_STAGE_CHANGED") { const triggerStage = cleanString(triggerConfig.stage, 80).toUpperCase(); if (flattened.some((action) => action.type === "UPDATE_CRM_STAGE" && action.value?.trim().toUpperCase() === triggerStage)) return { ok: false, error: "This workflow can set the same CRM stage that triggered it. Choose a different stage to avoid a loop." }; }
...
export function normalizeWhatsAppAutomationRow(row: Record<string, unknown>): WhatsAppAutomation {
  ...
  return { id: String(row.id || ""), ...value, version: Math.max(1, Number(row.version) || 1), createdByMemberId: typeof row.created_by_member_id === "string" ? row.created_by_member_id : undefined, updatedByMemberId: typeof row.updated_by_member_id === "string" ? row.updated_by_member_id : undefined, activatedAt: typeof row.activated_at === "string" ? row.activated_at : undefined, pausedAt: typeof row.paused_at === "string" ? row.paused_at : undefined, createdAt: typeof row.created_at === "string" ? row.created_at : undefined, updatedAt: typeof row.updated_at === "string" ? row.updated_at : undefined };
}
export function normalizeWhatsAppAutomationRunRow(row: Record<string, unknown>): WhatsAppAutomationRun {
  return { id: String(row.id || ""), automationId: String(row.automation_id || ""), automationVersion: Number(row.automation_version) || 1, status: String(row.status || "QUEUED") as WhatsAppAutomationRunStatus, triggerType: String(row.trigger_type || ""), triggerEventKey: typeof row.trigger_event_key === "string" ? row.trigger_event_key : undefined, contactId: typeof row.contact_id === "string" ? row.contact_id : undefined, conversationId: typeof row.conversation_id === "string" ? row.conversation_id : undefined, nextActionIndex: Number(row.next_action_index) || 0, startedAt: typeof row.started_at === "string" ? row.started_at : undefined, completedAt: typeof row.completed_at === "string" ? row.completed_at : undefined, errorCode: typeof row.error_code === "string" ? row.error_code : undefined, errorMessage: typeof row.error_message === "string" ? row.error_message : undefined, createdAt: typeof row.created_at === "string" ? row.created_at : undefined, updatedAt: typeof row.updated_at === "string" ? row.updated_at : undefined };
}
export function normalizeWhatsAppAutomationJobRow(row: Record<string, unknown>): WhatsAppAutomationJob {
  return { id: String(row.id || ""), runId: String(row.run_id || ""), automationId: String(row.automation_id || ""), status: String(row.status || "PENDING") as WhatsAppAutomationJobStatus, dueAt: String(row.due_at || ""), actionIndex: Number(row.action_index) || 0, attempts: Number(row.attempts) || 0, maxAttempts: Number(row.max_attempts) || 5, lastError: typeof row.last_error === "string" ? row.last_error : undefined, createdAt: typeof row.created_at === "string" ? row.created_at : undefined };
}
export function getWhatsAppAutomationTriggerLabel(type: WhatsAppAutomationTriggerType) { return WHATSAPP_AUTOMATION_TRIGGER_OPTIONS.find((item) => item.value === type)?.label || type; }
export function getWhatsAppAutomationActionLabel(type: WhatsAppAutomationActionType) { return WHATSAPP_AUTOMATION_ACTION_OPTIONS.find((item) => item.value === type)?.label || type; }
