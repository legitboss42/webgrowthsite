...
function generateVapidKeys(): VapidKeys { const ecdh = createECDH("prime256v1"); ecdh.generateKeys(); return { publicKey: ecdh.getPublicKey().toString("base64url"), privateKey: ecdh.getPrivateKey().toString("base64url") }; }
async function readVapidKeys(resolved: NonNullable<ReturnType<typeof config>>): Promise<VapidKeys | null> { const response = await fetch(`${resolved.url}/rest/v1/whatsapp_push_config?select=public_key,private_key&id=eq.default&limit=1`, { headers: headers(resolved.key), cache: "no-store" }); if (!response.ok) return null; const rows = await response.json() as Array<{ public_key?: string; private_key?: string }>; const row = rows[0]; return row?.public_key && row.private_key ? { publicKey: row.public_key, privateKey: row.private_key } : null; }
export async function ensureWhatsAppVapidKeys(): Promise<VapidKeys | null> {
  const resolved = config(); if (!resolved) return null; const existing = await readVapidKeys(resolved); if (existing) return existing;
...
export async function sendWhatsAppPushNotification(input: { id: string; title: string; body: string; url?: string }) {
  const keys = await ensureWhatsAppVapidKeys(); if (!keys) return { sent: 0, failed: 0 }; const subscriptions = await listSubscriptions(); if (!subscriptions.length) return { sent: 0, failed: 0 };
  webPush.setVapidDetails("mailto:admin@webgrowth.info", keys.publicKey, keys.privateKey);
  const payload = JSON.stringify({ id: input.id, title: input.title, body: input.body, url: input.url || "/admin/whatsapp/conversations/", icon: "/images/logo.webp", badge: "/favicon.ico" });
  let sent = 0; let failed = 0;
  await Promise.all(subscriptions.map(async (subscription) => { try { await webPush.sendNotification({ endpoint: subscription.endpoint, keys: { p256dh: subscription.p256dh, auth: subscription.auth } }, payload, { TTL: 300, urgency: "high" }); sent += 1; } catch (error) { failed += 1; const statusCode = error && typeof error === "object" && "statusCode" in error ? Number((error as { statusCode?: unknown }).statusCode) : 0; if (statusCode === 404 || statusCode === 410) await deleteWhatsAppPushSubscription(subscription.endpoint); else console.error("WhatsApp push delivery failed", { statusCode }); } }));
  return { sent, failed };
}
