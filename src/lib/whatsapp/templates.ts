...
export type WhatsAppTemplateCreateResult =
  ...
  | { ok: false; reason: "NOT_CONFIGURED" | "PERMISSION_DENIED" | "API_ERROR"; error?: string };

export type WhatsAppTemplateSendResult =
  | { ok: true; messageId: string }
  | { ok: false; reason: "NOT_CONFIGURED" | "INVALID_RECIPIENT" | "PERMISSION_DENIED" | "API_ERROR"; error?: string };
...
function getMetaConfig(env: Record<string, string | undefined>) {
  ...
  return { token, businessAccountId, phoneNumberId, apiVersion };
}
...
function classifyMetaPermission(status: number, code?: number) {
  return status === 401 || status === 403 || code === 10 || code === 200 || code === 190;
}
...
export function normalizeWhatsAppTemplate(raw: Record<string, unknown>): WhatsAppTemplate {
  ...
  const quality = raw.quality_score && typeof raw.quality_score === "object"
    ? readText((raw.quality_score as Record<string, unknown>).score)
    : readText(raw.quality_score);
  return {
    id: String(raw.id || ""),
    name: String(raw.name || ""),
    status: (KNOWN_STATUSES as string[]).includes(rawStatus) ? (rawStatus as WhatsAppTemplateStatus) : "UNKNOWN",
    category: readText(raw.category),
    language: readText(raw.language),
    rejectedReason: readText(raw.rejected_reason),
    qualityScore: quality,
    lastUpdatedTime: readText(raw.last_updated_time),
...
export function getWhatsAppTemplateComponent(template: WhatsAppTemplate, type: WhatsAppTemplateComponent["type"]) {
...
export function sortWhatsAppTemplates(templates: WhatsAppTemplate[]) {
  const rank = (status: WhatsAppTemplateStatus) => status === "APPROVED" ? 0 : status === "PENDING" ? 1 : status === "REJECTED" ? 2 : 3;
  return [...templates].sort((left, right) => rank(left.status) - rank(right.status) || left.name.localeCompare(right.name));
}
...
export async function fetchWhatsAppTemplates(options: MetaOptions & { limit?: number } = {}): Promise<WhatsAppTemplateFetchResult> {
  ...
  const { token, businessAccountId, apiVersion } = getMetaConfig(env);
  ...
  try {
    ...
    const response = await (options.fetch || globalThis.fetch)(
      `https://graph.facebook.com/${apiVersion}/${businessAccountId}/message_templates?limit=${limit}&fields=${fields}`,
      { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" },
    );
    if (!response.ok) {
      const detail = await readMetaError(response);
      console.error("WhatsApp template fetch failed", response.status, detail.code, detail.message);
      return { ok: false, reason: classifyMetaPermission(response.status, detail.code) ? "PERMISSION_DENIED" : "API_ERROR" };
    }
...
export async function createWhatsAppTemplate(input: WhatsAppTemplateDraftInput, options: MetaOptions = {}): Promise<WhatsAppTemplateCreateResult> {
  ...
  try {
    const response = await (options.fetch || globalThis.fetch)(
      `https://graph.facebook.com/${apiVersion}/${businessAccountId}/message_templates`,
      {
        ...
        body: JSON.stringify({
          ...
          components: buildWhatsAppMetaTemplateComponents(input),
        }),
      },
    );
    ...
    if (!response.ok || !payload?.id) {
      ...
      console.error("WhatsApp template create failed", response.status, code, payload?.error?.message);
      return {
        ok: false,
        reason: classifyMetaPermission(response.status, code) ? "PERMISSION_DENIED" : "API_ERROR",
        error: payload?.error?.message,
      };
    }
    return { ok: true, id: payload.id, status: payload.status, category: payload.category };
  } catch (error) {
    console.error("Unable to submit WhatsApp template", error);
    return { ok: false, reason: "API_ERROR" };
  }
}
...
export async function sendWhatsAppTemplateMessage(input: {
...
}, options: MetaOptions = {}): Promise<WhatsAppTemplateSendResult> {
  const env = options.env || process.env;
  const { token, phoneNumberId, apiVersion } = getMetaConfig(env);
  ...
  if (input.headerParameters?.length) {
    components.push({ type: "header", parameters: input.headerParameters.map((text) => ({ type: "text", text })) });
  }
  ...
  try {
    ...
    if (!response.ok || !messageId) {
      const code = payload?.error?.code;
      console.error("WhatsApp template send failed", response.status, code, payload?.error?.message);
      return {
        ok: false,
        reason: classifyMetaPermission(response.status, code) ? "PERMISSION_DENIED" : "API_ERROR",
        error: payload?.error?.message,
    ...
    console.error("Unable to send WhatsApp template", error);
    return { ok: false, reason: "API_ERROR" };
  }
}
