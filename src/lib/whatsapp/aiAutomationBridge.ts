...
import { generateWhatsAppAI, loadWhatsAppAISettings } from "./aiRuntime";
 ...
 *
 * Control tags are consumed after a routing/attempt so a human takeover stays a human
 * takeover and blocked AI requests cannot loop forever.
 */
export async function processWhatsAppAIAutomationTags(input: { waId: string }) {
  ...
  if (assignmentTag) {
    ...
    if (!settings.enabled || !settings.agentsEnabled) {
      ...
      await mutateWhatsAppRest({
        ...
        body: {
          ...
          event_type: "ai_agent_automation_blocked",
          metadata: { controlTag: assignmentTag, contactId, reason: !settings.enabled ? "AI_DISABLED" : "AGENTS_DISABLED" },
        },
      });
    } else {
