...
export type WhatsAppInteractiveQuestionInput = {
  to: string;
  question: string;
  mode: WhatsAppAutomationQuestionMode;
...
export function buildWhatsAppInteractiveQuestionPayload(input: WhatsAppInteractiveQuestionInput) {
  ...
  if (input.mode === "BUTTONS") {
    return {
      ...
      interactive: {
        ...
        body: { text: body },
        action: {
          buttons: input.choices.slice(0, 3).map((choice) => ({
            type: "reply",
  ...
  return {
    ...
    interactive: {
      ...
      body: { text: body },
      action: {
        button: input.listButtonText?.trim() || "Choose",
        sections: [{
          rows: input.choices.slice(0, 10).map((choice) => ({
            id: choice.id,
            title: choice.title,
...
export async function sendWhatsAppInteractiveQuestion(input: WhatsAppInteractiveQuestionInput, options: Options = {}): Promise<SendResult> {
  ...
  try {
    const response = await (options.fetch || globalThis.fetch)(`https://graph.facebook.com/${apiVersion}/${phoneNumberId}/messages`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify(buildWhatsAppInteractiveQuestionPayload({ ...input, to: recipient })),
    });
    ...
    const diagnostic = { status: response.status, ...(typeof code === "number" ? { code } : {}) };
