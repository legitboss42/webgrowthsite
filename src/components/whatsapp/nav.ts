...
/**
 * Navigation model for the WhatsApp console shell.
 *
 * `status` is deliberately explicit: only routes that actually exist are marked
 * "live" and rendered as links. Everything else renders as a disabled row with a
 * "Soon" chip, so the shell never points at a 404 or implies a feature exists.
 */
...
export const WHATSAPP_NAV_SECTIONS: WhatsAppNavSection[] = [
  ...
  },
  {
    ...
    items: [
      {
        label: "Campaigns",
        href: `${WHATSAPP_CONSOLE_ROOT}/campaigns`,
        icon: "campaigns",
        description: "Outbound template broadcasts.",
        status: "soon",
      },
      {
        label: "Automations",
        href: `${WHATSAPP_CONSOLE_ROOT}/automations`,
        icon: "automations",
        description: "Auto-replies and lead routing rules.",
        status: "soon",
      },
      {
        label: "Analytics",
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: "Settings",
        href: `${WHATSAPP_CONSOLE_ROOT}/settings`,
        icon: "settings",
        description: "Integration configuration and connection readiness.",
        status: "live",
      },
    ],
  },
...
export const WHATSAPP_NAV_ITEMS: WhatsAppNavItem[] = WHATSAPP_NAV_SECTIONS.flatMap(
  (section) => section.items,
);

/** Trailing slashes vary across the app's own links, so compare normalised paths. */
...
export function isWhatsAppNavItemActive(pathname: string | null | undefined, href: string) {
  ...
  // The console root must not stay highlighted while a child route is open.
...
export function findWhatsAppNavItem(pathname: string | null | undefined) {
  return (
    WHATSAPP_NAV_ITEMS.find((item) => isWhatsAppNavItemActive(pathname, item.href)) || null
  );
}
