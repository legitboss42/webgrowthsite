...
function Brand({ href, compact = false, onNavigate }: { href: string; compact?: boolean; onNavigate?: () => void }) {
  return (
    <a href={href} onClick={onNavigate} aria-label="Open WhatsApp overview" className={`wg-brand flex min-w-0 items-center ${compact ? "justify-center" : "gap-2.5"}`}>
...
function SidebarContent({ senderConnected, senderNumber, role, memberName, workspaceControl, onNavigate, compact = false }: {
  senderConnected: boolean;
  senderNumber?: string;
  role: WhatsAppTeamRole;
  memberName: string;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className={`wg-sidebar-brand flex-none pb-2 pt-1 ${compact ? "px-1.5" : "px-2"}`}>
        <div className={`wg-brand-row flex min-w-0 items-center ${compact ? "justify-center gap-0.5" : "gap-1"}`}>
          <Brand href={workspaceHome(role)} compact={compact} onNavigate={onNavigate} />
          {workspaceControl ? <div className="wg-brand-workspace-control flex-none">{workspaceControl}</div> : null}
        </div>
      </div>
  ...
  return (
    <div className={`wg-whatsapp-app flex min-w-0 text-ink ${fillsViewport ? "h-dvh overflow-hidden" : "min-h-dvh"}`}>
      <aside className={`wg-app-sidebar relative sticky top-0 hidden h-dvh flex-none flex-col py-3 transition-[width] duration-200 lg:flex ${sidebarCollapsed ? "w-[4.5rem]" : "w-[14.5rem]"}`}>
        <button type="button" onClick={() => setSidebarCollapsed((value) => !value)} aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} className="absolute -right-3 top-4 z-20 grid h-7 w-7 place-items-center rounded-full border border-white/[.09] bg-[#0b1511] text-white/55 shadow-lg transition hover:bg-[#12231b] hover:text-white">
          <WhatsAppIcon name="chevronLeft" className={`h-3.5 w-3.5 transition-transform ${sidebarCollapsed ? "rotate-180" : ""}`} />
        </button>
        <SidebarContent compact={sidebarCollapsed} senderConnected={senderConnected} senderNumber={senderNumber} role={role} memberName={memberName} workspaceControl={workspaceControl} />
      </aside>
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside id={drawerId} role="dialog" aria-modal="true" aria-label="WhatsApp console navigation" className="wg-app-sidebar absolute inset-y-0 left-0 flex w-[18rem] max-w-[88vw] flex-col py-3 shadow-2xl">
            <button type="button" onClick={() => setDrawerOpen(false)} className="absolute right-3 top-3 z-10 grid h-8 w-8 place-items-center rounded-lg border border-white/[.06] bg-white/[.035] text-white/45"><WhatsAppIcon name="close" className="h-4 w-4" /><span className="sr-only">Close navigation</span></button>
            <SidebarContent senderConnected={senderConnected} senderNumber={senderNumber} role={role} memberName={memberName} workspaceControl={workspaceControl} onNavigate={() => setDrawerOpen(false)} />
          </aside>
        </div>
      ) : null}
      ...
      <div className={`flex min-w-0 max-w-full flex-1 flex-col ${fillsViewport ? "overflow-hidden" : ""}`}>
        <header className="wg-app-topbar sticky top-0 z-30 flex-none">
          <div className="wg-topbar-inner flex min-h-[3.65rem] flex-wrap items-center gap-2 px-3 py-2 sm:px-4 lg:flex-nowrap lg:gap-3 lg:px-5">
            <button type="button" onClick={() => setDrawerOpen(true)} aria-expanded={drawerOpen} aria-controls={drawerId} className="wg-topbar-menu grid h-9 w-9 place-items-center rounded-lg border border-white/[.07] bg-white/[.025] text-white/60 lg:hidden"><WhatsAppIcon name="menu" className="h-4.5 w-4.5" /><span className="sr-only">Open navigation</span></button>
            <div className="wg-topbar-mobile-brand flex min-w-0 items-center gap-0.5 lg:hidden"><Brand href={workspaceHome(role)} compact />{workspaceControl ? <div className="wg-brand-workspace-control flex-none">{workspaceControl}</div> : null}</div>
            <div className="wg-topbar-title min-w-0 flex-1">
              <div className="flex min-w-0 items-center gap-2"><h1 className="truncate text-sm font-semibold text-white/90 sm:text-[0.94rem]">{pageMeta.title}</h1>{workspaceName ? <span className="hidden truncate text-[0.65rem] text-white/28 sm:inline lg:hidden">/ {workspaceName}</span> : null}</div>
              <p className="mt-0.5 hidden truncate text-[0.68rem] text-white/30 md:block">{pageMeta.description}</p>
            </div>
            <div className="wg-topbar-status flex flex-none items-center gap-2">
            ...
            <div id="wg-whatsapp-topbar-actions" className="wg-topbar-actions order-4 flex min-w-0 basis-full items-center justify-end gap-1.5 overflow-x-auto border-t border-white/[.06] pt-2 empty:hidden lg:order-none lg:basis-auto lg:border-0 lg:pt-0" aria-label="Conversation actions" />
          </div>
        </header>
