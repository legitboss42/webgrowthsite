...
function NavRow({ item, onNavigate }: { item: WhatsAppNavItem; onNavigate?: () => void }) {
  ...
  if (item.status === "soon") {
    return (
      <span aria-disabled="true" title={`${item.description} Coming in a later stage.`} className="flex cursor-not-allowed items-center gap-3 rounded-lg px-3 py-2 text-sm text-white/35">
        <WhatsAppIcon name={item.icon} />
        <span className="truncate">{item.label}</span>
        <span className="ml-auto rounded-full border border-white/10 px-2 py-0.5 text-[0.625rem] font-medium uppercase tracking-[.12em] text-white/40">Soon</span>
      </span>
    );
  }

  return (
    <Link href={item.href} onClick={onNavigate} aria-current={active ? "page" : undefined} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition ${active ? "bg-ledger-bright font-medium text-white shadow-[0_6px_16px_-8px_rgba(28,122,84,.9)]" : "text-white/70 hover:bg-white/10 hover:text-white"}`}>
      <WhatsAppIcon name={item.icon} />
      <span className="truncate">{item.label}</span>
    </Link>
  );
    ...
    <div className="flex h-full flex-col">
      ...
      <nav aria-label="WhatsApp console" className="mt-5 flex-1 overflow-y-auto pb-4">
        {sections.map((section) => (
          <div key={section.label} className="mb-1">
            <p className="px-3 pb-1.5 pt-3 text-[0.625rem] font-semibold uppercase tracking-[.2em] text-white/35">{section.label}</p>
      ...
      <div className="mt-auto border-t border-white/10 pt-3">
        <div className="mb-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
          <p className="truncate text-xs font-semibold text-white/85">{memberName}</p>
          <p className="mt-0.5 text-[0.625rem] uppercase tracking-[.14em] text-white/45">{role}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
          <div className="flex items-center gap-2.5">
            <span aria-hidden="true" className={`h-2 w-2 flex-none rounded-full ${senderConnected ? "bg-emerald-400 shadow-[0_0_0_3px_rgba(52,211,153,.2)]" : "bg-white/30"}`} />
            <div className="min-w-0">
              <p className="text-[0.625rem] uppercase tracking-[.14em] text-white/45">{getWhatsAppSenderStatusText(senderConnected)}</p>
              {senderNumber ? <p className="truncate font-mono text-xs text-white/85">{senderNumber}</p> : null}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`flex bg-paper text-ink ${fillsViewport ? "h-dvh overflow-hidden" : "min-h-dvh"}`}>
      <aside className="sticky top-0 hidden h-dvh w-64 flex-none flex-col bg-ledger-deep px-3 py-4 lg:flex">
        <SidebarContent senderConnected={senderConnected} senderNumber={senderNumber} role={role} memberName={memberName} />
      </aside>

      {drawerOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" aria-label="Close navigation" onClick={() => setDrawerOpen(false)} className="absolute inset-0 h-full w-full bg-ink/50 backdrop-blur-[2px]" />
          <div id={drawerId} role="dialog" aria-modal="true" aria-label="WhatsApp console navigation" className="absolute inset-y-0 left-0 flex w-[17rem] max-w-[85vw] flex-col bg-ledger-deep px-3 py-4 shadow-2xl">
            <div className="mb-1 flex justify-end">
              <button type="button" onClick={() => setDrawerOpen(false)} className="rounded-lg p-2 text-white/70 transition hover:bg-white/10 hover:text-white">
                <WhatsAppIcon name="close" />
                <span className="sr-only">Close navigation</span>
              </button>
            </div>
            <SidebarContent senderConnected={senderConnected} senderNumber={senderNumber} role={role} memberName={memberName} onNavigate={() => setDrawerOpen(false)} />
          </div>
        </div>
      ) : null}
      ...
      <div className={`flex min-w-0 flex-1 flex-col ${fillsViewport ? "overflow-hidden" : ""}`}>
        <header className="sticky top-0 z-30 flex-none border-b border-rule bg-paper-raised/95 backdrop-blur supports-[backdrop-filter]:bg-paper-raised/80">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-3 px-4 py-3 sm:px-6">
            <button type="button" onClick={() => setDrawerOpen(true)} aria-expanded={drawerOpen} aria-controls={drawerId} className="-ml-1 rounded-lg p-2 text-ink-soft transition hover:bg-paper-sunk hover:text-ink lg:hidden">
              <WhatsAppIcon name="menu" className="h-5 w-5" />
              <span className="sr-only">Open navigation</span>
            </button>

            <Link href={workspaceHome(role)} aria-label="Web Growth WhatsApp console" className="flex-none rounded-md p-1 transition hover:bg-paper-sunk lg:hidden">
              <Image src="/images/brand/web-growth-logo.webp" alt="Web Growth" width={270} height={40} sizes="120px" quality={75} className="h-4 w-auto" />
            </Link>

            <div className="min-w-0 flex-1">
              <h1 className="font-display text-lg font-semibold leading-tight text-ink sm:text-xl">{title}</h1>
