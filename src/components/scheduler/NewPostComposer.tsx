...
export default function NewPostComposer({ owner, articles = [] }: { owner: boolean; articles?: Array<{slug:string;title:string}> }) {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      if (owner && article) {
        const generated = await fetch("/api/scheduler/articles/", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ slug: article }),
        });
        const post = await generated.json();
        if (!generated.ok) throw new Error(post.error);
  ...
  return <form onSubmit={handleSubmit} className="mt-9 space-y-6 rounded-[2rem] border border-white/10 bg-white/[.035] p-6 sm:p-8">
    ...
    <label className="block"><span className="mb-2 block text-sm text-white/60">One original video or up to 10 photos</span><input name="media" type="file" multiple accept="video/mp4,video/quicktime,video/webm,image/jpeg,image/png,image/webp" className="w-full rounded-xl border border-dashed border-white/20 p-5" /></label>
