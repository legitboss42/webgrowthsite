...
import { mutateWhatsAppRest } from "@/app/admin/whatsapp/data";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "INIT") {
      return encryptedResponse({ version: WHATSAPP_FLOW_DATA_API_VERSION, screen: text(screens[0].id), data: merged }, aesKey, iv);
    }
