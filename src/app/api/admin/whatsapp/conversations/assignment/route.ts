...
export async function POST(request: Request) {
  ...
  const conversationRows = await readWhatsAppRows<Record<string, unknown>>(
    `whatsapp_conversations?id=eq.${encodeURIComponent(conversationId)}&select=id,assigned_member_id,assigned_to&limit=1`,
  );
  ...
  const currentMemberId = typeof conversation.assigned_member_id === "string"
  ...
  if (requestedMemberId) {
    ...
    if (!targetMember) {
      return NextResponse.json({ error: "That team member is not active." }, { status: 409 });
    }
  }
  ...
  if (!canWhatsAppRoleSuperviseTeam(access.role)) {
    ...
    if (!targetMember || targetMember.id !== access.memberId) {
      return NextResponse.json({ error: "Agents can only assign an unassigned conversation to themselves." }, { status: 403 });
    }
    if (currentMemberId && currentMemberId !== access.memberId) {
      return NextResponse.json({ error: "This conversation is already assigned to another team member." }, { status: 409 });
    }
  }
  ...
  if (assignmentIsUnchanged) {
    return NextResponse.json({
      ...
      assignment: targetMember
        ? { memberId: targetMember.id, displayName: targetMember.displayName, googleEmail: targetMember.googleEmail }
        : null,
    });
  }
  ...
  const updated = await mutateWhatsAppRest({
    ...
    body: {
      ...
      assigned_to: targetMember?.googleEmail || null,
