...
import {
  isWhatsAppPlatformAdmin,
  resolveWhatsAppWorkspaceForIdentity,
...
/** Legacy gate for truly platform-global surfaces. Client workspace owners should use getWhatsAppWorkspaceAccess instead. */
...
async function resolveIdentityWorkspace(input: {
  email: string;
  displayName: string;
  source: WhatsAppWorkspaceAccess["source"];
  ...
  if (platformAdmin && workspace.isPlatformOwned && !member) {
    member = await ensureWhatsAppOwnerTeamMember({ email: input.email, displayName: input.displayName, workspaceId: workspace.id });
  }
  if (!platformAdmin && !member) return null;

  return {
    role: platformAdmin ? "owner" : member!.role,
    memberId: member?.id || null,
    email: input.email,
    displayName: member?.displayName || input.displayName || input.email,
    source: input.source,
...
export async function getWhatsAppWorkspaceAccess(cookieStore: CookieStoreLike): Promise<WhatsAppWorkspaceAccess | null> {
  try {
    ...
    if (googleSession) {
      return await resolveIdentityWorkspace({
        email: googleSession.email,
        displayName: googleSession.fullName || googleSession.email,
        source: "google",
        cookieStore,
        configuredPlatformAdmin: isGoogleAdminSession(googleSession),
      });
    }
  } catch {}
  ...
  try {
    ...
    if (passwordSession) {
      return await resolveIdentityWorkspace({
        ...
        displayName: passwordSession.fullName || passwordSession.email,
        source: "password",
        cookieStore,
        configuredPlatformAdmin: isAllowedGoogleAdminEmail(passwordSession.email),
      });
    }
  } catch {}
...
export async function hasWhatsAppPlatformAdminAccess(cookieStore: CookieStoreLike) {
  const access = await getWhatsAppWorkspaceAccess(cookieStore);
