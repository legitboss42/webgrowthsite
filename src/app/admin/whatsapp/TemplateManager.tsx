...
export default function TemplateManager({ liveTemplates, drafts, draftsReady, canManage, role, liveError }: { liveTemplates: WhatsAppTemplate[]; drafts: WhatsAppTemplateDraft[]; draftsReady: boolean; canManage: boolean; role: "owner" | "manager" | "agent"; liveError?: string }) {
  ...
  const visibleLive = useMemo(() => liveTemplates.filter((template) => { if (status !== "ALL" && template.status !== status) return false; const q = query.trim().toLowerCase(); return !q || [template.name, template.status, template.category, template.language, template.rejectedReason].some((v) => v?.toLowerCase().includes(q)); }), [liveTemplates, query, status]);
